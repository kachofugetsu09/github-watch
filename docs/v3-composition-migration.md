...
- Core 依赖：Akashic Core 提供 `tools.v1`、`programmatic.v1`、`core.timers`、正式 runtime lifecycle 与 typed `TurnCommitted`。

## Goal

把旧 runtime/Tool 类和宿主 side effect 收成为纯 v3 composition：由 `tools.v1` 登记五个声明，
由 `core.timers` 与 runtime lifecycle 持有正式轮询任务，并登记 typed `TurnCommitted` listener。
...
- Core 拥有 one-shot Timer、stable snapshot lease、Session/Turn identity 与 admission、
  `tools.v1` registry/执行上下文、typed TurnCommitted 的触发时机和 generation 晋升。
...
- `github_watch.ProgrammaticTurnPort` 是插件领域端口；插件通过 `programmatic.v1` 的
  `session/admit` 与 `message/send` 适配它，并要求 send 返回真实 accepted `turn_id`。
...
- programmatic Turn 返回 accepted receipt 后立即结束本轮轮询，不等待模型结果；

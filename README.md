...
- 默认每 120 秒由 Core Timer 检查一次；ETag 减少重复列表传输，稳定事件键和 SQLite 账本避免重复处理。
...
- 每个 Issue/PR 通过 Agent Input 复用同一 Session；每次都会生成新的完整证据包。
...
插件只注入 `core.timer`、`core.agent_input` 和 `core.tools`。轮询、Session 准入、Tool 声明和
Turn 完成事件由 Core 提供边界；GitHub 客户端、SQLite、证据、checkout、幂等与重试仍由插件实现。
...
该版本要求包含 stable Timer snapshot scheduling、stable Agent Input admission 与 async serial
TurnCommitted stage 的 Akashic Core。
